...
function getAuthHeaders() {
    // 浠巄lobal 璁剧疆鐨刢oken鎴杢ookie/sessionStorage涓疄鏃惰幏鍙?
    let token = authToken;

    // 鍏堣 auth_bootstrap.js 鍦═indow 涓敤娉勬鍊?
    if ((!token || token === 'null' || token === 'undefined') && typeof window !== 'undefined') {
        if (window.authToken) {
            token = window.authToken;
        }
    }

    // 濡傛灉杩樻棤 token锛屽啀浠巃essionStorage 閲岄潰鎼滅储
    if ((!token || token === 'null' || token === 'undefined') && typeof window !== 'undefined') {
        try {
